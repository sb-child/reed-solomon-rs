...
pub struct Decoder<const ECCLEN: usize> {}

impl<const ECCLEN: usize> Decoder<ECCLEN> {
    ...
    pub fn correct_err_count(
        ...
        let mut msg = Buffer::from_slice(msg, msg.len() - ECCLEN);
        ...
        if self.is_corrupted(&msg_out) {
            ...
            Ok((Buffer::from_polynom(msg_out, msg.len() - ECCLEN), fixed))
        }
    }
    ...
    /// assert_eq!(&[1, 2, 3, 4], corrected.data())
    /// ```
    pub fn correct(&self, msg: &[u8], erase_pos: Option<&[u8]>) -> Result<Buffer> {
    ...
    pub fn is_corrupted(&self, msg: &[u8]) -> bool {
        (0..ECCLEN).any(|x| msg.eval(gf::pow(2, x as i32)) != 0)
    }

    fn calc_syndromes(&self, msg: &[u8]) -> Polynom {
        // index 0 is a pad for mathematical precision
        let mut synd = Polynom::with_length(ECCLEN + 1);
        for i in 0..ECCLEN {
    ...
    fn correct_errata(&self, msg: &[u8], synd: &[u8], err_pos: &[u8]) -> (Polynom, usize) {
        ...
        let err_eval = self
            .find_error_evaluator(&synd.reverse(), &err_loc, err_loc.len() - 1)
            .reverse();
    ...
    fn find_error_locator(
        ...
        let synd_shift = if synd.len() > ECCLEN {
            synd.len() - ECCLEN
        } else {
            0
        };

        for i in 0..(ECCLEN - erase_count) {
...
mod tests {
    use super::*;
    use crate::FixedEncoder;

    #[test]
    fn calc_syndromes() {
        let px = [1, 2, 3, 4, 5, 6, 7, 8, 9];
        let mut encoded = FixedEncoder::<8>::new().encode(&px[..]);
    ...
    fn correct_errata() {
        let msg = [
            0, 0, 0, 2, 2, 2, 119, 111, 114, 108, 100, 145, 124, 96, 105, 94, 31, 179, 149, 163,
        ];
        let synd = [0, 64, 42, 242, 59, 109, 56, 78, 103, 232];
        let err_pos = [0, 1, 2, 5, 4, 3];
        let result = [
            104, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100, 145, 124, 96, 105, 94, 31, 179,
            149, 163,
        ];
    ...
    fn error_count() {
        ...
        let encoder = FixedEncoder::<10>::new();
